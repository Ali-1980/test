...
#define MAX_TIMEOUT_SECONDS 30.0
...
- (BOOL)prepareBackupDirectory:(NSError **)error {
    ...
    // 确保备份结构
    BOOL structureValid = [self ensureBackupStructure:error];
    if (!structureValid) {
        [self logError:@"备份目录结构验证失败"];
        return NO;
    }

    [self logInfo:@"备份目录准备完成，验证通过"];
    return YES;
}
...
- (BOOL)checkDeviceState:(NSError **)error {
    ...
    // ===== 设备锁定状态检查 =====
    BOOL isDeviceLocked = NO;  // 跟踪锁定状态
    
    // 方法1: 直接检查DeviceLocked属性
    ...
    // 方法2: 如果无法获取锁定状态且错误为-12，认为设备已锁定
    else if (lerr == LOCKDOWN_E_PASSWORD_PROTECTED) {
        [self logInfo:@"设备已锁定 (无法读取DeviceLocked，错误码:-12)"];
        isDeviceLocked = YES;
    }
    
    // 方法3: 检查其他敏感属性，如果错误为-12，同样认为设备已锁定
    if (!isDeviceLocked) {
        ...
        if (lerr == LOCKDOWN_E_PASSWORD_PROTECTED) {
            [self logInfo:@"设备已锁定 (无法读取HostAttached，错误码:-12)"];
            isDeviceLocked = YES;
        }
        else if (lerr == LOCKDOWN_E_SUCCESS && trustState) {
            // 如果成功获取了信任状态，检查值
            if (plist_get_node_type(trustState) == PLIST_BOOLEAN) {
                uint8_t isTrusted = 0;
                plist_get_bool_val(trustState, &isTrusted);
                
                if (!isTrusted) {
                    [self logInfo:@"设备未被完全信任 (HostAttached=false)"];
                    
                    // 显示信任提示
                    dispatch_async(dispatch_get_main_queue(), ^{
                        NSAlert *alert = [[NSAlert alloc] init];
                        [alert setMessageText:@"需要设备信任"];
                    ...
                    if (error) {
                        *error = [self createError:MFCToolBackupErrorTrustNotEstablished
                                      description:@"设备未信任此电脑，请在设备上确认信任"];
                    }
        ...
        // 其他错误但不是-12，仅记录信息
            ...
            // 不是锁定问题但无法获取信任状态，记录信息但仍可继续
            [self logInfo:@"无法获取信任状态详情(错误码:%d)，将尝试继续备份", lerr];
        }
    }
    
    // 如果确定设备已锁定，显示提示并返回错误
    if (isDeviceLocked) {
        [self logInfo:@"设备已锁定，无法继续备份"];
        
        // 显示解锁提示
        dispatch_async(dispatch_get_main_queue(), ^{
            NSAlert *alert = [[NSAlert alloc] init];
            [alert setMessageText:@"设备已锁定"];
            [alert setInformativeText:@"请解锁iOS设备后重试备份。"];
            [alert addButtonWithTitle:@"好的"];
            [alert runModal];
        });
    ...
    // 所有检查通过
    [self logInfo:@"设备状态检查结果：设备已解锁且可以通信，继续备份"];
    return YES;
}
...
- (BOOL)sendBackupRequest:(NSError **)error {
    ...
    dispatch_sync(self.connectionQueue, ^{
        ...
        [self logInfo:@"创建备份选项字典"];
        
        // 基本选项
        plist_dict_set_item(options, "TargetIdentifier", plist_new_string([_backupContext->deviceUDID UTF8String]));
        [self logInfo:@"设置目标标识符: %@", _backupContext->deviceUDID];
        
        // 强制使用完全备份 - 修改这里，确保参数正确
        plist_dict_set_item(options, "FullBackup", plist_new_bool(1));
        plist_dict_set_item(options, "IncrementalBackup", plist_new_bool(0));
        [self logInfo:@"备份类型: 强制完全备份"];
        
        // 设置备份路径
        plist_dict_set_item(options, "BackupComputerBase", plist_new_string([_backupContext->backupPath UTF8String]));
        [self logInfo:@"设置备份路径: %@", _backupContext->backupPath];
        
        // 重要：设置DLFileOffsetKey标志
        plist_dict_set_item(options, "DLFileOffsetKey", plist_new_bool(0));
        [self logInfo:@"设置DLFileOffsetKey: FALSE"];
        
        // 加密选项
        if (_backupContext->isEncrypted) {
            // 明确设置加密标志
            plist_dict_set_item(options, "WillEncrypt", plist_new_bool(1));
            [self logInfo:@"设置加密标志: TRUE"];
            
            // 如果有密码，添加密码
            if (_backupContext->password) {
                plist_dict_set_item(options, "Password", plist_new_string([_backupContext->password UTF8String]));
                [self logInfo:@"添加加密密码到备份选项, 密码长度: %lu", (unsigned long)[_backupContext->password length]];
                
                // 添加加密版本信息
                plist_dict_set_item(options, "PasswordKey", plist_new_string("EncryptionKey"));
                plist_dict_set_item(options, "EncryptionType", plist_new_uint(3)); // iOS标准加密类型
                [self logInfo:@"设置加密类型和密钥标识符"];
            } else {
                [self logWarning:@"备份被标记为加密但没有提供密码"];
            }
        } else {
            // 明确设置不加密
            plist_dict_set_item(options, "WillEncrypt", plist_new_bool(0));
            [self logInfo:@"设置加密标志: FALSE"];
        }
        
        // 添加高级选项
        plist_dict_set_item(options, "BackupApplications", plist_new_bool(1));
        [self logInfo:@"设置备份应用: TRUE"];
        
        // 添加额外选项 - 这可能对触发文件上传很重要
        plist_dict_set_item(options, "BackupSystemFiles", plist_new_bool(1));
        [self logInfo:@"设置备份系统文件: TRUE"];
        
        // 确保设置了这些关键选项
        plist_dict_set_item(options, "iTunesFiles", plist_new_bool(1));
        plist_dict_set_item(options, "CameraRollDomain", plist_new_bool(1));
        plist_dict_set_item(options, "MediaDomain", plist_new_bool(1));
        [self logInfo:@"设置备份媒体文件和相机胶卷: TRUE"];
        ...
        [self logInfo:@"设置备份协议版本: %s", version_str];
        
        // 打印完整选项以便调试
        char *xml = NULL;
        uint32_t length = 0;
        plist_to_xml(options, &xml, &length);
        if (xml) {
            [self logInfo:@"完整备份选项: %s", xml];
            free(xml);
        }
        
        // 发送备份请求
        [self logInfo:@"发送备份请求到设备..."];
        ...
        if (mb2_err != MOBILEBACKUP2_E_SUCCESS) {
            [self logError:@"发送备份请求失败，错误代码: %d, 错误描述: %s", mb2_err, mobilebackup2_strerror(mb2_err)];
            localError = [self createError:[self mapProtocolErrorToErrorCode:mb2_err]
                              description:[NSString stringWithFormat:@"发送备份请求失败，错误代码: %d, 错误描述: %s", mb2_err, mobilebackup2_strerror(mb2_err)]];
            success = NO;
            return;
        }
        
        [self logInfo:@"备份请求已成功发送"];
        success = YES;
    });
    ...
    [self logInfo:@"发送备份请求 - 结束，结果: %@", success ? @"成功" : @"失败"];
    return success;
}

// 在类实现中添加此辅助函数，用于获取错误字符串
const char* mobilebackup2_strerror(mobilebackup2_error_t err) {
    switch (err) {
...
// 在类实现中添加此辅助函数，用于检查常见问题
- (BOOL)checkCommonBackupIssues {
    [self logInfo:@"检查常见备份问题..."];
    BOOL hasIssues = NO;
    ...
    if (_backupContext && _backupContext->lockdown) {
        plist_t trustState = NULL;
        lockdownd_error_t lerr = lockdownd_get_value(_backupContext->lockdown, "com.apple.mobile.lockdown", "TrustLevel", &trustState);
        
        if (lerr == LOCKDOWN_E_SUCCESS && trustState) {
            char *trust_str = NULL;
            if (plist_get_node_type(trustState) == PLIST_STRING) {
                plist_get_string_val(trustState, &trust_str);
                if (trust_str) {
                    [self logInfo:@"设备信任级别: %s", trust_str];
                    if (strcmp(trust_str, "Complete") != 0) {
                        [self logWarning:@"设备信任级别不完整: %s", trust_str];
                    }
                    free(trust_str);
                }
            }
            ...
            [self logWarning:@"无法获取设备信任状态: %d", lerr];
        }
    }
    
    // 3. 检查备份服务是否正常启动
    if (!_backupContext || !_backupContext->backup) {
        [self logError:@"备份服务未正常启动"];
        hasIssues = YES;
    }
    
    // 4. 检查协议版本
    if (_backupContext && _backupContext->protocolVersion < 2.0) {
        [self logWarning:@"协议版本较低: %.1f，可能不支持所有功能", _backupContext->protocolVersion];
    }
    
    // 5. 检查iOS版本兼容性
    if (_backupContext && _backupContext->lockdown) {
        plist_t productVersion = NULL;
        lockdownd_error_t lerr = lockdownd_get_value(_backupContext->lockdown, NULL, "ProductVersion", &productVersion);
        
        if (lerr == LOCKDOWN_E_SUCCESS && productVersion) {
            char *version_str = NULL;
            if (plist_get_node_type(productVersion) == PLIST_STRING) {
                plist_get_string_val(productVersion, &version_str);
                if (version_str) {
                    [self logInfo:@"iOS版本: %s", version_str];
                    free(version_str);
                }
            }
            plist_free(productVersion);
        }
    }
    
    [self logInfo:@"常见问题检查%@", hasIssues ? @"发现问题" : @"通过"];
    return !hasIssues;
}
...
- (BOOL)isIncrementalBackupCompleted {
    ...
    NSString *dbPath = [_backupContext->backupPath stringByAppendingPathComponent:@"Manifest.db"];
    ...
    [self logInfo:@"备份文件检查 - Manifest.plist: %@, Info.plist: %@, Status.plist: %@, Manifest.db: %@",
    ...
    // 如果基本文件不完整，则不是有效备份
    if (!manifestExists || !infoExists || !statusExists || !dbExists) {
    ...
    if (![self validatePlistFiles:&plistError]) {
        [self logWarning:@"plist文件验证失败: %@", plistError.localizedDescription];
        return NO;
    }
    
    // 验证数据库结构
    NSError *dbError = nil;
    if (![self validateManifestDatabase:dbPath error:&dbError]) {
        [self logWarning:@"数据库验证失败: %@", dbError.localizedDescription];
        return NO;
    }
    ...
    if (!status) {
        [self logWarning:@"无法读取Status.plist"];
        return NO;
    }
    
    // 检查备份状态
    NSNumber *backupState = status[@"BackupState"];
    NSString *snapshotState = status[@"SnapshotState"];
    
    if ((backupState && [backupState intValue] == 0) ||
        (snapshotState && [snapshotState isEqualToString:@"finished"])) {
        [self logInfo:@"Status.plist表明备份已完成"];
    } else {
        [self logWarning:@"Status.plist表明备份未完成，状态: %@, 快照状态: %@",
                    backupState ? [backupState stringValue] : @"未知",
                    snapshotState ?: @"未知"];
        return NO;
    }
    
    // 检查Info.plist中的UDID是否匹配
    NSDictionary *info = [NSDictionary dictionaryWithContentsOfFile:infoPath];
    
    if (!info) {
        [self logWarning:@"无法读取Info.plist"];
        return NO;
    }
    ...
    // 排除基本文件后还应该有其他文件
    NSMutableSet *basicFiles = [NSMutableSet setWithArray:@[@"Info.plist", @"Manifest.plist", @"Status.plist", @"Manifest.db", @"backup_log.txt"]];
    ...
    if (rc != SQLITE_OK) {
        [self logWarning:@"无法打开数据库: %s", sqlite3_errmsg(db)];
        sqlite3_close(db);
        return NO;
    }
    ...
    if (fileCount == 0) {
        [self logWarning:@"数据库中没有文件记录"];
        return NO;
    }
    
    [self logInfo:@"数据库中包含 %d 个文件记录", fileCount];
...
- (BOOL)processBackupMessages:(NSError **)error {
    ...
    // 自适应超时设置 - 使用更短的初始超时
    NSTimeInterval initialTimeout = 10.0;  // 缩短初始超时为3秒
    NSTimeInterval currentTimeout = initialTimeout;
    NSTimeInterval maxTimeout = 30.0;     // 减少最大超时为15秒
    NSTimeInterval minTimeout = 5.0;      // 设置最小超时2秒
    ...
    while (!backupCompleted && [self shouldContinue]) {
        ...
        if (messageReceived) {
            ...
            // 处理接收到的消息
            if (dlmessage) {
                [self logInfo:@"收到消息类型: %s (#%d)", dlmessage, messageSequence];
...
- (void)handleUploadFiles:(plist_t)message {
    ...
    if (!entries || plist_get_node_type(entries) != PLIST_ARRAY) {
        // 尝试其他键名
        entries = plist_dict_get_item(message, "Data");
        if (!entries) {
            entries = plist_dict_get_item(message, "Files");
        }
    }
    
    if (entries && (plist_get_node_type(entries) == PLIST_ARRAY ||
                   plist_get_node_type(entries) == PLIST_DICT)) {
        uint32_t entryCount = 0;
        if (plist_get_node_type(entries) == PLIST_ARRAY) {
            entryCount = plist_array_get_size(entries);
        } else {
            entryCount = plist_dict_get_size(entries);
        }
        
        [self logInfo:@"收到的文件条目数量: %u", entryCount];
        ...
        for (uint32_t i = 0; i < entryCount && [self shouldContinue]; i++) {
            ...
            if (path) {
                ...
                [self logDebug:@"处理文件路径: %s", pathStr];
            } else {
                [self logWarning:@"条目 #%u 缺少路径", i];
            }
            
            if (data && plist_get_node_type(data) == PLIST_DATA) {
                plist_get_data_val(data, &dataBytes, &dataSize);
                [self logDebug:@"文件数据大小: %llu 字节", dataSize];
            } else {
                [self logWarning:@"条目 #%u 缺少数据或数据类型错误", i];
            }
            ...
            if (pathStr && dataBytes && dataSize > 0) {
                ...
                [self logInfo:@"处理文件: %@, 大小: %llu 字节", [fullPath lastPathComponent], dataSize];
                    ...
                    @autoreleasepool {
                        ...
                        if (![[NSFileManager defaultManager] createDirectoryAtPath:dirPath
                                                      withIntermediateDirectories:YES
                                                                       attributes:nil
                                                                            error:&dirError]) {
                            [self logWarning:@"创建目录失败: %@ - %@", dirPath, dirError.localizedDescription];
                        }
                        ...
                        if (success) {
                            ...
                            dispatch_sync(self.stateQueue, ^{
                                ...
                                // 每50个文件记录一次进度
                                if (self.filesProcessed % 50 == 0) {
                                    [self logInfo:@"已处理 %d 个文件，总数据量: %lld 字节",
                ...
                if (!pathStr) [self logWarning:@"路径为空"];
                if (!dataBytes || dataSize == 0) [self logWarning:@"数据为空或长度为0"];
            }
        ...
        [self logWarning:@"上传消息缺少Entries字段或格式错误"];
        
        // 检查消息中的其他字段
        plist_dict_iter iter = NULL;
        plist_dict_new_iter(message, &iter);
        if (iter) {
            char *key = NULL;
            plist_t val = NULL;
            
            [self logInfo:@"上传消息中的字段:"];
            
            while (1) {
                plist_dict_next_item(message, iter, &key, &val);
                if (!key) break;
                
                [self logInfo:@" - 字段: %s, 类型: %d", key, plist_get_node_type(val)];
                free(key);
            }
    ...
    dispatch_sync(self.connectionQueue, ^{
        if (_backupContext && _backupContext->backup) {
            [self logInfo:@"发送文件上传响应..."];
            plist_t response = plist_new_dict();
            
            // 添加更多响应信息
            plist_dict_set_item(response, "Status", plist_new_string("Complete"));
            
            mobilebackup2_error_t resp_err = mobilebackup2_send_status_response(_backupContext->backup, 0, NULL, response);
            if (resp_err != MOBILEBACKUP2_E_SUCCESS) {
                [self logWarning:@"发送上传响应失败: %d", resp_err];
            } else {
                [self logInfo:@"上传响应发送成功"];
            }
            
            plist_free(response);
        } else {
            [self logWarning:@"无法发送上传响应: 备份客户端无效"];
        }
    });
    ...
    if (elapsed > 0 && bytesReceived > 0) {
        ...
        [self logInfo:@"文件传输速度: %.2f MB/s, 接收数据: %lld 字节, 耗时: %.1f 秒",
                 speedMBps, bytesReceived, elapsed];
    } else {
        [self logInfo:@"文件传输完成，但无数据接收或耗时为0"];
    }
    
    [self logInfo:@"处理文件上传请求 - 结束"];
}
...
- (void)handleProcessMessage:(plist_t)message {
    ...
    if (self.logLevel >= MFCLogLevelDebug) {
        ...
        plist_to_xml(message, &xml, &length);
        if (xml) {
            [self logDebug:@"接收到的ProcessMessage内容: %s", xml];
            free(xml);
        }
    }
    ...
    if (messageNameStr) {
        ...
        dispatch_sync(self.connectionQueue, ^{
            if (_backupContext && _backupContext->backup) {
                ...
                if (strcmp(messageNameStr, "BackupMessageBackupReady") == 0) {
                    ...
                    [self logDebug:@"已发送BackupReady响应"];
                }
                else if (strcmp(messageNameStr, "BackupMessageShowBackupState") == 0) {
                    plist_dict_set_item(response, "BackupState", plist_new_string("Idle"));
                    plist_dict_set_item(response, "Result", plist_new_string("Success"));
                    mobilebackup2_send_message(_backupContext->backup, messageNameStr, response);
                    [self logDebug:@"已发送BackupState响应"];
                }
                else if (strcmp(messageNameStr, "BackupMessageStatus") == 0) {
                    plist_dict_set_item(response, "Status", plist_new_string("Complete"));
                    plist_dict_set_item(response, "Result", plist_new_string("Success"));
                    mobilebackup2_send_message(_backupContext->backup, messageNameStr, response);
                    [self logDebug:@"已发送Status响应"];
                }
                else {
                    // 通用响应处理
                    plist_dict_set_item(response, "Status", plist_new_string("Complete"));
                    plist_dict_set_item(response, "Result", plist_new_string("Success"));
                    mobilebackup2_send_message(_backupContext->backup, messageNameStr, response);
                    [self logDebug:@"已发送通用响应: %s", messageNameStr];
                }
                
                plist_free(response);
            } else {
                [self logWarning:@"备份上下文或客户端为空，无法发送响应"];
            }
        });
        ...
        [self logWarning:@"过程消息名称为空，发送通用状态响应"];
        ...
        dispatch_sync(self.connectionQueue, ^{
            if (_backupContext && _backupContext->backup) {
                ...
                // 检查是否有可用的错误信息或状态
                plist_t status = plist_dict_get_item(message, "Status");
                if (status && (plist_get_node_type(status) == PLIST_STRING)) {
                    char *statusStr = NULL;
                    plist_get_string_val(status, &statusStr);
                    if (statusStr) {
                        [self logInfo:@"消息中包含状态: %s", statusStr];
                        plist_dict_set_item(response, "Status", plist_new_string(statusStr));
                        free(statusStr);
                    }
                } else {
                    // 默认状态
...
// 改进后的createManifestDatabase方法

- (BOOL)createManifestDatabase:(NSString *)backupPath error:(NSError **)error {
    [self logInfo:@"创建Manifest数据库: %@", [backupPath stringByAppendingPathComponent:@"Manifest.db"]];
    
    NSString *dbPath = [backupPath stringByAppendingPathComponent:@"Manifest.db"];
    sqlite3 *db;
    
    // 打开数据库文件，如果不存在则创建
    int rc = sqlite3_open([dbPath UTF8String], &db);
    if (rc != SQLITE_OK) {
        [self logError:@"无法创建Manifest.db: %s", sqlite3_errmsg(db)];
        sqlite3_close(db);
        
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"无法创建数据库: %s", sqlite3_errmsg(db)]];
        }
        return NO;
    }
    ...
    if (rc != SQLITE_OK) {
        [self logError:@"开始事务失败: %s", errMsg];
        sqlite3_free(errMsg);
        sqlite3_close(db);
        
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"开始事务失败: %s", errMsg]];
        }
        return NO;
    }
    
    // 创建Files表，包含wrappedKey字段用于加密备份
    const char *filesSql =
        "CREATE TABLE Files ("
        "  fileID TEXT PRIMARY KEY, "
        "  domain TEXT, "
        "  relativePath TEXT, "
        "  flags INTEGER, "  // 1=文件, 2=目录, 4=符号链接
        "  file BLOB, "      // 文件元数据
        "  wrappedKey BLOB"  // 加密备份使用的密钥
        ");";
    
    rc = sqlite3_exec(db, filesSql, NULL, NULL, &errMsg);
    if (rc != SQLITE_OK) {
        [self logError:@"创建Files表失败: %s", errMsg];
        sqlite3_free(errMsg);
        ...
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"创建Files表失败: %s", errMsg]];
        }
        return NO;
    }
    
    // 创建索引以提高查询性能
    const char *indexesSql =
        "CREATE INDEX DomainIndex ON Files(domain);"
        "CREATE INDEX PathIndex ON Files(relativePath);";
    
    rc = sqlite3_exec(db, indexesSql, NULL, NULL, &errMsg);
    if (rc != SQLITE_OK) {
        [self logError:@"创建索引失败: %s", errMsg];
        sqlite3_free(errMsg);
        sqlite3_exec(db, "ROLLBACK;", NULL, NULL, NULL);
        sqlite3_close(db);
        ...
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"创建索引失败: %s", errMsg]];
        }
        return NO;
    }
    
    // 创建Properties表用于存储备份元数据
    const char *propertiesSql =
        "CREATE TABLE Properties ("
        "  key TEXT PRIMARY KEY, "
        "  value BLOB"
        ");";
    
    rc = sqlite3_exec(db, propertiesSql, NULL, NULL, &errMsg);
    if (rc != SQLITE_OK) {
        [self logError:@"创建Properties表失败: %s", errMsg];
        sqlite3_free(errMsg);
        sqlite3_exec(db, "ROLLBACK;", NULL, NULL, NULL);
        sqlite3_close(db);
        ...
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"创建Properties表失败: %s", errMsg]];
        }
        return NO;
    }
    
    // 添加版本信息
    const char *versionSql = "INSERT INTO Properties (key, value) VALUES ('VERSION', '4.0');";
    rc = sqlite3_exec(db, versionSql, NULL, NULL, &errMsg);
    if (rc != SQLITE_OK) {
        [self logError:@"添加版本信息失败: %s", errMsg];
        sqlite3_free(errMsg);
        sqlite3_exec(db, "ROLLBACK;", NULL, NULL, NULL);
        sqlite3_close(db);
        ...
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"添加版本信息失败: %s", errMsg]];
        }
        return NO;
    }

    // 添加加密状态 - 检查_backupContext的isEncrypted属性
    BOOL isEncrypted = _backupContext && _backupContext->isEncrypted;
    sqlite3_stmt *stmt;
    const char *encryptSql = "INSERT INTO Properties (key, value) VALUES ('IsEncrypted', ?);";
    rc = sqlite3_prepare_v2(db, encryptSql, -1, &stmt, NULL);
    
    if (rc == SQLITE_OK) {
        sqlite3_bind_int(stmt, 1, isEncrypted ? 1 : 0);
        rc = sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        
        if (rc != SQLITE_DONE) {
            [self logError:@"添加加密状态失败: %s", sqlite3_errmsg(db)];
            sqlite3_exec(db, "ROLLBACK;", NULL, NULL, NULL);
            sqlite3_close(db);
            
            if (error) {
                *error = [self createError:MFCToolBackupErrorDatabaseOperation
                              description:[NSString stringWithFormat:@"添加加密状态失败: %s", sqlite3_errmsg(db)]];
            }
            return NO;
        }
    }
    ...
    if (rc != SQLITE_OK) {
        ...
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"提交事务失败: %s", errMsg]];
        }
        return NO;
    }
    ...
    [self logInfo:@"成功创建Manifest数据库"];
    return YES;
}

// 添加一个辅助方法用于验证数据库结构
- (BOOL)validateManifestDatabase:(NSString *)dbPath error:(NSError **)error {
    sqlite3 *db;
    ...
    if (rc != SQLITE_OK) {
        if (error) {
            ...
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                           description:[NSString stringWithFormat:@"无法打开数据库: %@", sqliteError]];
        }
        sqlite3_close(db);
        return NO;
    }
    ...
    const char *checkTablesSQL = "SELECT name FROM sqlite_master WHERE type='table' AND (name='Files' OR name='Properties')";
    sqlite3_stmt *stmt;
    
    rc = sqlite3_prepare_v2(db, checkTablesSQL, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        tablesValid = NO;
    } else {
...
// 添加一个将文件添加到备份数据库的方法
- (BOOL)addFileToBackupDb:(NSString *)fileID
    ...
    if (rc != SQLITE_OK) {
        [self logError:@"无法打开Manifest.db: %s", sqlite3_errmsg(db)];
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"无法打开数据库: %s", sqlite3_errmsg(db)]];
        }
        sqlite3_close(db);
        return NO;
    }
    
    // 准备SQL语句 - 根据是否有wrappedKey选择不同的SQL
    sqlite3_stmt *stmt;
    const char *sql;
    
    if (wrappedKey) {
        sql = "INSERT OR REPLACE INTO Files (fileID, domain, relativePath, flags, file, wrappedKey) VALUES (?, ?, ?, ?, ?, ?)";
    } else {
        sql = "INSERT OR REPLACE INTO Files (fileID, domain, relativePath, flags, file) VALUES (?, ?, ?, ?, ?)";
    }
    
    rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    
    if (rc != SQLITE_OK) {
        [self logError:@"准备SQL语句失败: %s", sqlite3_errmsg(db)];
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"准备SQL语句失败: %s", sqlite3_errmsg(db)]];
        }
        sqlite3_close(db);
        return NO;
    }
    ...
    // 绑定参数
    sqlite3_bind_text(stmt, 1, [fileID UTF8String], -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, [domain UTF8String], -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, [relativePath UTF8String], -1, SQLITE_TRANSIENT);
    sqlite3_bind_int(stmt, 4, flags);
    
    if (metadata) {
        sqlite3_bind_blob(stmt, 5, [metadata bytes], (int)[metadata length], SQLITE_TRANSIENT);
    } else {
    ...
    // 如果有包装密钥，绑定它
    if (wrappedKey) {
        sqlite3_bind_blob(stmt, 6, [wrappedKey bytes], (int)[wrappedKey length], SQLITE_TRANSIENT);
    }
    ...
    if (rc != SQLITE_DONE) {
        [self logError:@"执行SQL语句失败: %s", sqlite3_errmsg(db)];
        if (error) {
            *error = [self createError:MFCToolBackupErrorDatabaseOperation
                          description:[NSString stringWithFormat:@"执行SQL语句失败: %s", sqlite3_errmsg(db)]];
        }
        sqlite3_close(db);
        return NO;
    }
...
// 添加清理或修复已损坏数据库的方法
- (BOOL)repairManifestDatabase:(NSString *)backupPath error:(NSError **)error {
    [self logInfo:@"尝试修复Manifest数据库"];
    
    NSString *dbPath = [backupPath stringByAppendingPathComponent:@"Manifest.db"];
    NSString *backupDbPath = [dbPath stringByAppendingString:@".bak"];
    ...
    if ([fileManager fileExistsAtPath:dbPath]) {
        ...
        if (![fileManager copyItemAtPath:dbPath toPath:backupDbPath error:&copyError]) {
            [self logWarning:@"无法创建数据库备份: %@", copyError.localizedDescription];
            // 继续尝试修复
        }
    }
    ...
    if ([fileManager fileExistsAtPath:dbPath]) {
        ...
        if (![fileManager removeItemAtPath:dbPath error:&removeError]) {
            [self logError:@"无法删除损坏的数据库: %@", removeError.localizedDescription];
            if (error) {
                *error = [self createError:MFCToolBackupErrorFileOperation
                              description:[NSString stringWithFormat:@"无法删除损坏的数据库: %@",
    ...
    // 创建新的数据库
    BOOL success = [self createManifestDatabase:backupPath error:error];
    
    // 如果有备份，尝试从备份中恢复数据
    if (success && [fileManager fileExistsAtPath:backupDbPath]) {
        sqlite3 *oldDb;
        sqlite3 *newDb;
        
        int rc1 = sqlite3_open([backupDbPath UTF8String], &oldDb);
        int rc2 = sqlite3_open([dbPath UTF8String], &newDb);
        
        if (rc1 != SQLITE_OK || rc2 != SQLITE_OK) {
            [self logWarning:@"无法从备份恢复数据"];
            sqlite3_close(oldDb);
            sqlite3_close(newDb);
        } else {
            // 尝试从旧数据库导入数据
            [self logInfo:@"尝试从备份恢复数据"];
            
            // 开始新数据库的事务
            sqlite3_exec(newDb, "BEGIN TRANSACTION;", NULL, NULL, NULL);
            
            // 尝试获取旧数据库中的文件
            sqlite3_stmt *stmt;
            const char *sql = "SELECT fileID, domain, relativePath, flags, file, wrappedKey FROM Files LIMIT 10000";
            
            if (sqlite3_prepare_v2(oldDb, sql, -1, &stmt, NULL) == SQLITE_OK) {
                int recordCount = 0;
                int successCount = 0;
                ...
                while (sqlite3_step(stmt) == SQLITE_ROW) {
                    ...
                    const char *relativePath = (const char *)sqlite3_column_text(stmt, 2);
                    int flags = sqlite3_column_int(stmt, 3);
                    
                    const void *fileData = sqlite3_column_blob(stmt, 4);
                    int fileSize = sqlite3_column_bytes(stmt, 4);
                    
                    const void *keyData = sqlite3_column_blob(stmt, 5);
                    int keySize = sqlite3_column_bytes(stmt, 5);
                    ...
                    if (keySize > 0) {
                        insertSql = "INSERT OR REPLACE INTO Files (fileID, domain, relativePath, flags, file, wrappedKey) VALUES (?, ?, ?, ?, ?, ?)";
                    } else {
                        insertSql = "INSERT OR REPLACE INTO Files (fileID, domain, relativePath, flags, file) VALUES (?, ?, ?, ?, ?)";
                    }
                    
                    if (sqlite3_prepare_v2(newDb, insertSql, -1, &insertStmt, NULL) == SQLITE_OK) {
                        // 绑定参数
                        if (fileID) sqlite3_bind_text(insertStmt, 1, fileID, -1, SQLITE_TRANSIENT);
                        if (domain) sqlite3_bind_text(insertStmt, 2, domain, -1, SQLITE_TRANSIENT);
                        if (relativePath) sqlite3_bind_text(insertStmt, 3, relativePath, -1, SQLITE_TRANSIENT);
                        sqlite3_bind_int(insertStmt, 4, flags);
                        
                        if (fileData && fileSize > 0) {
                            sqlite3_bind_blob(insertStmt, 5, fileData, fileSize, SQLITE_TRANSIENT);
                        } else {
                            sqlite3_bind_null(insertStmt, 5);
                        }
                        
                        if (keyData && keySize > 0) {
                            sqlite3_bind_blob(insertStmt, 6, keyData, keySize, SQLITE_TRANSIENT);
                        }
                ...
                [self logInfo:@"从备份恢复了 %d/%d 条记录", successCount, recordCount];
                sqlite3_finalize(stmt);
            }
            
            // 提交事务
            sqlite3_exec(newDb, "COMMIT;", NULL, NULL, NULL);
        ...
        // 删除备份数据库
        [fileManager removeItemAtPath:backupDbPath error:nil];
    }
    
    if (!success && error) {
        *error = [self createError:MFCToolBackupErrorDatabaseOperation
                      description:@"修复Manifest数据库失败"];
    }
...
- (BOOL)ensureBackupStructure:(NSError **)error {
    ...
    dispatch_sync(self.fileQueue, ^{
        ...
        dbExists = [fileManager fileExistsAtPath:dbPath];
    });
    
    [self logDebug:@"检查基本文件 - Info.plist: %@, Manifest.plist: %@, Status.plist: %@, Manifest.db: %@",
               infoExists ? @"已存在" : @"不存在",
    ...
    // 创建或验证Manifest.db
    if (!dbExists) {
        [self logInfo:@"创建Manifest.db数据库"];
        NSError *dbError = nil;
        if (![self createManifestDatabase:_backupContext->backupPath error:&dbError]) {
            [self logError:@"创建Manifest.db失败: %@", dbError.localizedDescription];
            if (error) {
                *error = dbError;
            }
            return NO;
        }
        [self logInfo:@"成功创建Manifest.db"];
    } else {
        // 验证数据库是否有效
        [self logInfo:@"验证现有Manifest.db"];
        NSError *validationError = nil;
        if (![self validateManifestDatabase:dbPath error:&validationError]) {
            [self logWarning:@"现有Manifest.db无效，尝试修复: %@", validationError.localizedDescription];
            
            // 尝试修复数据库
            NSError *repairError = nil;
            if (![self repairManifestDatabase:_backupContext->backupPath error:&repairError]) {
                [self logError:@"修复Manifest.db失败: %@", repairError.localizedDescription];
                if (error) {
                    *error = repairError;
                }
                return NO;
            }
            [self logInfo:@"成功修复Manifest.db"];
        } else {
            [self logInfo:@"Manifest.db验证通过"];
        }
    }
    
    // 验证plist文件内容
    if (![self validatePlistFiles:error]) {
        [self logError:@"验证plist文件失败"];
        return NO;
    }
...
//用于验证plist文件内容
- (BOOL)validatePlistFiles:(NSError **)error {
    [self logInfo:@"验证plist文件内容"];
    ...
    if (!infoDict) {
        [self logError:@"无法读取Info.plist或文件内容无效"];
        if (error) {
            *error = [self createError:MFCToolBackupErrorFileOperation
                          description:@"Info.plist内容无效"];
        }
        return NO;
    }
    
    // 检查Info.plist必要键
    NSArray *requiredInfoKeys = @[@"UDID", @"BackupComputerName", @"Date", @"Version"];
    for (NSString *key in requiredInfoKeys) {
        if (![infoDict objectForKey:key]) {
            [self logError:@"Info.plist缺少必要键: %@", key];
            if (error) {
                *error = [self createError:MFCToolBackupErrorFileOperation
                              description:[NSString stringWithFormat:@"Info.plist缺少必要键: %@", key]];
            }
            return NO;
        }
    }
    ...
    if (!manifestDict) {
        [self logError:@"无法读取Manifest.plist或文件内容无效"];
        if (error) {
            *error = [self createError:MFCToolBackupErrorFileOperation
                          description:@"Manifest.plist内容无效"];
        }
        return NO;
    }
    
    // 检查Manifest.plist必要键
    NSArray *requiredManifestKeys = @[@"Version", @"IsEncrypted"];
    for (NSString *key in requiredManifestKeys) {
        if (![manifestDict objectForKey:key]) {
            [self logError:@"Manifest.plist缺少必要键: %@", key];
            if (error) {
                *error = [self createError:MFCToolBackupErrorFileOperation
                              description:[NSString stringWithFormat:@"Manifest.plist缺少必要键: %@", key]];
            }
            return NO;
        }
    }
    ...
    // 验证Status.plist
    NSDictionary *statusDict = [NSDictionary dictionaryWithContentsOfFile:statusPath];
    if (!statusDict) {
        [self logError:@"无法读取Status.plist或文件内容无效"];
        if (error) {
            *error = [self createError:MFCToolBackupErrorFileOperation
                          description:@"Status.plist内容无效"];
        }
        return NO;
    }
    
    // 检查Status.plist必要键
    NSArray *requiredStatusKeys = @[@"UUID", @"BackupState", @"Date"];
    for (NSString *key in requiredStatusKeys) {
        if (![statusDict objectForKey:key]) {
            [self logError:@"Status.plist缺少必要键: %@", key];
            if (error) {
                *error = [self createError:MFCToolBackupErrorFileOperation
                              description:[NSString stringWithFormat:@"Status.plist缺少必要键: %@", key]];
            }
            return NO;
        }
    }
    
    [self logInfo:@"所有plist文件验证通过"];
    return YES;
}
...
- (void)finishWithSuccess {
    ...
    // 执行全面备份验证
    NSError *validationError = nil;
    if (![self validateCompleteBackup:&validationError]) {
        [self logError:@"备份验证失败: %@", validationError.localizedDescription];
        [self finishWithError:validationError];
        return;
    }
...
//全面验证备份
- (BOOL)validateCompleteBackup:(NSError **)error {
    [self logInfo:@"执行全面备份验证"];
    
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *backupPath = _backupContext->backupPath;
    
    // 1. 验证基本文件存在
    NSArray *requiredFiles = @[@"Info.plist", @"Manifest.plist", @"Status.plist", @"Manifest.db"];
    for (NSString *fileName in requiredFiles) {
        NSString *filePath = [backupPath stringByAppendingPathComponent:fileName];
        if (![fileManager fileExistsAtPath:filePath]) {
            [self logError:@"缺少必要文件: %@", fileName];
            if (error) {
                *error = [self createError:MFCToolBackupErrorIncomplete
                              description:[NSString stringWithFormat:@"备份缺少必要文件: %@", fileName]];
            }
            return NO;
        }
    }
    ...
    // 3. 验证数据库结构
    NSString *dbPath = [backupPath stringByAppendingPathComponent:@"Manifest.db"];
    if (![self validateManifestDatabase:dbPath error:error]) {
        return NO;
    }
    
    // 4. 更新Status.plist的备份状态为完成
    NSString *statusPath = [backupPath stringByAppendingPathComponent:@"Status.plist"];
    NSMutableDictionary *statusDict = [NSMutableDictionary dictionaryWithContentsOfFile:statusPath];
    if (statusDict) {
        statusDict[@"BackupState"] = @(0); // 0表示备份完成
        statusDict[@"SnapshotState"] = @"finished";
        statusDict[@"Date"] = @([[NSDate date] timeIntervalSince1970]);
        
        if (![statusDict writeToFile:statusPath atomically:YES]) {
            [self logWarning:@"无法更新Status.plist状态"];
        }
    }
    ...
    BOOL isEncrypted = _backupContext && _backupContext->isEncrypted;
    if (isEncrypted) {
        NSString *manifestPath = [backupPath stringByAppendingPathComponent:@"Manifest.plist"];
        NSMutableDictionary *manifestDict = [NSMutableDictionary dictionaryWithContentsOfFile:manifestPath];
        
        if (manifestDict) {
            // 确保加密标志正确设置
            if (![[manifestDict objectForKey:@"IsEncrypted"] boolValue]) {
                manifestDict[@"IsEncrypted"] = @YES;
...
typedef NS_ENUM(NSInteger, MFCToolBackupError) {
    ...
    MFCToolBackupErrorDatabaseOperation = 21
};
...
2025-05-14 12:49:57.041576+0800 MFCTOOL.bin[94396:4779889] [triggerStartBackup INFO] 收到传递的设备 deviceUDID: 00008110-001025522E41801E 模式: Normal
2025-05-14 12:49:57.057221+0800 MFCTOOL.bin[94396:4779889] [DEBUG] 已保存操作日志: 00008110-001025522E41801E -> 设备备份操作
2025-05-14 12:49:57.057340+0800 MFCTOOL.bin[94396:4779889] [DEBUG] Core Data 数据库存储路径: /Users/monterey/Library/Application Support/com.mfcbox.imfcdata/OperationLogs/OperationHistory.sqlite
2025-05-14 12:49:57.058814+0800 MFCTOOL.bin[94396:4779914] 设备连接测试成功，开始执行备份任务
2025-05-14 12:49:57.059871+0800 MFCTOOL.bin[94396:4779914] [INFO] BackupTask初始化完成
2025-05-14 12:49:57.060313+0800 MFCTOOL.bin[94396:4779914] [INFO] 启动备份任务，设备UDID: 00008110-001025522E41801E
2025-05-14 12:49:57.060439+0800 MFCTOOL.bin[94396:4779914] [INFO] 备份上下文创建成功，UDID: 00008110-001025522E41801E
2025-05-14 12:49:57.060561+0800 MFCTOOL.bin[94396:4779914] [INFO] 备份任务已分派到后台队列
2025-05-14 12:49:57.060763+0800 MFCTOOL.bin[94396:4779897] [INFO] 开始异步备份任务
2025-05-14 12:49:57.060841+0800 MFCTOOL.bin[94396:4779897] [INFO] === 开始内部备份流程 ===
2025-05-14 12:49:57.060877+0800 MFCTOOL.bin[94396:4779897] [INFO] 步骤1: 准备备份目录 - 开始
2025-05-14 12:49:57.061139+0800 MFCTOOL.bin[94396:4779897] [INFO] 准备备份目录
2025-05-14 12:49:57.061324+0800 MFCTOOL.bin[94396:4779897] [INFO] 完整备份路径: /Users/monterey/Library/Application Support/com.mfcbox.imfcdata/backups/00008110-001025522E41801E
2025-05-14 12:49:57.061367+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份目录已存在
2025-05-14 12:49:57.062753+0800 MFCTOOL.bin[94396:4779897] [INFO] 确保备份目录结构完整
2025-05-14 12:49:57.062893+0800 MFCTOOL.bin[94396:4779897] [INFO] 验证现有Manifest.db
2025-05-14 12:49:57.063611+0800 MFCTOOL.bin[94396:4779897] [INFO] Manifest.db验证通过
2025-05-14 12:49:57.063658+0800 MFCTOOL.bin[94396:4779897] [INFO] 验证plist文件内容
2025-05-14 12:49:57.064156+0800 MFCTOOL.bin[94396:4779897] [INFO] 所有plist文件验证通过
2025-05-14 12:49:57.064195+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份目录准备完成，验证通过
2025-05-14 12:49:57.064224+0800 MFCTOOL.bin[94396:4779897] [INFO] 步骤1: 准备备份目录 - 完成
2025-05-14 12:49:57.065140+0800 MFCTOOL.bin[94396:4779897] [INFO] 步骤2: 连接设备 - 开始
2025-05-14 12:49:57.065233+0800 MFCTOOL.bin[94396:4779897] [INFO] 连接设备: 00008110-001025522E41801E
2025-05-14 12:49:57.065370+0800 MFCTOOL.bin[94396:4779889] Progress: 1.0%, 连接设备...
2025-05-14 12:49:57.067877+0800 MFCTOOL.bin[94396:4779897] [INFO] 设备连接成功
2025-05-14 12:49:57.107079+0800 MFCTOOL.bin[94396:4779897] [INFO] Lockdown客户端创建成功
2025-05-14 12:49:57.108546+0800 MFCTOOL.bin[94396:4779897] [INFO] 设备名称: Denis
2025-05-14 12:49:57.110521+0800 MFCTOOL.bin[94396:4779897] [INFO] 成功保存设备配对信息
2025-05-14 12:49:57.110608+0800 MFCTOOL.bin[94396:4779897] [INFO] 成功保存设备配对信息
2025-05-14 12:49:57.112688+0800 MFCTOOL.bin[94396:4779897] [INFO] 无法获取信任状态详情(错误码:-12)，将尝试继续备份
2025-05-14 12:49:57.113988+0800 MFCTOOL.bin[94396:4779897] [INFO] 设备电量: 76%
2025-05-14 12:49:57.114051+0800 MFCTOOL.bin[94396:4779897] [INFO] 设备状态检查结果：设备已解锁且可以通信，继续备份
2025-05-14 12:49:57.114100+0800 MFCTOOL.bin[94396:4779897] [INFO] 步骤2: 连接设备 - 完成
2025-05-14 12:49:57.114146+0800 MFCTOOL.bin[94396:4779897] [INFO] 步骤3: 初始化服务 - 开始
2025-05-14 12:49:57.114190+0800 MFCTOOL.bin[94396:4779897] [INFO] 初始化服务
2025-05-14 12:49:57.114289+0800 MFCTOOL.bin[94396:4779889] Progress: 3.0%, 初始化服务...
2025-05-14 12:49:57.207284+0800 MFCTOOL.bin[94396:4779897] [INFO] MobileBackup2服务已准备就绪
2025-05-14 12:49:57.207468+0800 MFCTOOL.bin[94396:4779897] [INFO] 步骤3: 初始化服务 - 完成
2025-05-14 12:49:57.207551+0800 MFCTOOL.bin[94396:4779897] [INFO] 步骤4: 执行备份 - 开始
2025-05-14 12:49:57.207630+0800 MFCTOOL.bin[94396:4779897] [INFO] 执行备份流程
2025-05-14 12:49:57.207706+0800 MFCTOOL.bin[94396:4779897] [INFO] 执行协议版本协商
2025-05-14 12:49:57.207840+0800 MFCTOOL.bin[94396:4779898] [INFO] 状态转换: 初始化中 -> 协议协商中
2025-05-14 12:49:57.209344+0800 MFCTOOL.bin[94396:4779897] [INFO] 协议版本协商成功，远程版本: 2.1
2025-05-14 12:49:57.209451+0800 MFCTOOL.bin[94396:4779897] [INFO] 获取设备信息
2025-05-14 12:49:57.209488+0800 MFCTOOL.bin[94396:4779889] Progress: 8.0%, 协议协商完成: 2.1
2025-05-14 12:49:57.209593+0800 MFCTOOL.bin[94396:4779889] Progress: 8.0%, 获取设备信息...
2025-05-14 12:49:57.246109+0800 MFCTOOL.bin[94396:4779897] [INFO] 通过AFC获取设备容量: 238 GB, 估计备份大小: 47 GB
2025-05-14 12:49:57.246365+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份加密状态: 启用
2025-05-14 12:49:57.246620+0800 MFCTOOL.bin[94396:4779897] [INFO] 处理加密备份
2025-05-14 12:49:57.246687+0800 MFCTOOL.bin[94396:4779889] Progress: 9.0%, 设备信息获取完成
2025-05-14 12:49:57.246725+0800 MFCTOOL.bin[94396:4779897] [INFO] 请求备份密码
2025-05-14 12:49:57.246902+0800 MFCTOOL.bin[94396:4779898] [INFO] 状态转换: 协议协商中 -> 等待密码
2025-05-14 12:50:02.236129+0800 MFCTOOL.bin[94396:4779897] [INFO] 已获取备份密码
2025-05-14 12:50:02.236260+0800 MFCTOOL.bin[94396:4779897] [INFO] 发送备份请求 - 开始
2025-05-14 12:50:02.236328+0800 MFCTOOL.bin[94396:4779897] [INFO] 创建备份选项字典
2025-05-14 12:50:02.236384+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置目标标识符: 00008110-001025522E41801E
2025-05-14 12:50:02.236432+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份类型: 强制完全备份
2025-05-14 12:50:02.236510+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份路径: /Users/monterey/Library/Application Support/com.mfcbox.imfcdata/backups/00008110-001025522E41801E
2025-05-14 12:50:02.236561+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置DLFileOffsetKey: FALSE
2025-05-14 12:50:02.236605+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置加密标志: TRUE
2025-05-14 12:50:02.236658+0800 MFCTOOL.bin[94396:4779897] [INFO] 添加加密密码到备份选项, 密码长度: 4
2025-05-14 12:50:02.236705+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置加密类型和密钥标识符
2025-05-14 12:50:02.236750+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份应用: TRUE
2025-05-14 12:50:02.236792+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份系统文件: TRUE
2025-05-14 12:50:02.236847+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份媒体文件和相机胶卷: TRUE
2025-05-14 12:50:02.236893+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份协议版本: 2.1
2025-05-14 12:50:02.236961+0800 MFCTOOL.bin[94396:4779897] [INFO] 完整备份选项: <?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>TargetIdentifier</key>
    <string>00008110-001025522E41801E</string>
    <key>FullBackup</key>
    <true/>
    <key>IncrementalBackup</key>
    <false/>
    <key>BackupComputerBase</key>
    <string>/Users/monterey/Library/Application Support/com.mfcbox.imfcdata/backups/00008110-001025522E41801E</string>
    ...
    <integer>3</integer>
    <key>BackupApplications</key>
    <true/>
    <key>BackupSystemFiles</key>
    <true/>
    <key>iTunesFiles</key>
    <true/>
    <key>CameraRollDomain</key>
    <true/>
    <key>MediaDomain</key>
    <true/>
    <key>Version</key>
...
2025-05-14 12:50:02.242811+0800 MFCTOOL.bin[94396:4779897] [INFO] 发送备份请求到设备...
2025-05-14 12:50:02.243089+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份请求已成功发送
2025-05-14 12:50:02.243141+0800 MFCTOOL.bin[94396:4779897] [INFO] 发送备份请求 - 结束，结果: 成功
2025-05-14 12:50:02.243192+0800 MFCTOOL.bin[94396:4779897] [INFO] 开始处理备份消息...
2025-05-14 12:50:02.243252+0800 MFCTOOL.bin[94396:4779915] [INFO] 状态转换: 等待密码 -> 备份中
2025-05-14 12:50:02.296850+0800 MFCTOOL.bin[94396:4779889] Progress: 8.0%, 等待密码输入...
2025-05-14 12:50:03.491082+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 开始备份...
2025-05-14 12:50:03.491295+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:04.584077+0800 MFCTOOL.bin[94396:4779897] [INFO] 收到消息类型: DLMessageDownloadFiles (#1)
2025-05-14 12:50:04.584352+0800 MFCTOOL.bin[94396:4779897] [INFO] 处理文件下载请求 (#1)
2025-05-14 12:50:04.584464+0800 MFCTOOL.bin[94396:4779897] [INFO] 处理文件下载请求
2025-05-14 12:50:04.586370+0800 MFCTOOL.bin[94396:4779897] [INFO] 文件下载请求处理完成
2025-05-14 12:50:06.327470+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:09.352991+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:12.591508+0800 MFCTOOL.bin[94396:4779897] [WAR] 接收消息超时 (8.0秒)，超时次数: 1/5
2025-05-14 12:50:13.352949+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:16.552942+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:20.463533+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:23.552802+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:26.559502+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:30.552370+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:34.552328+0800 MFCTOOL.bin[94396:4779889] Progress: 10.0%, 备份中 - 已处理 0/0 文件，传输: 0.00 B/47.66 GB (10.0%)
2025-05-14 12:50:34.589594+0800 MFCTOOL.bin[94396:4779897] [INFO] 增加接收超时时间到 10.4秒
2025-05-14 12:50:34.589760+0800 MFCTOOL.bin[94396:4779897] [INFO] 超时后尝试重置备份客户端连接
2025-05-14 12:50:34.589824+0800 MFCTOOL.bin[94396:4779897] [INFO] 开始重建备份连接
2025-05-14 12:50:34.683716+0800 MFCTOOL.bin[94396:4779897] [INFO] 成功创建新的备份客户端
2025-05-14 12:50:34.684736+0800 MFCTOOL.bin[94396:4779897] [INFO] 协议版本协商成功，远程版本: 2.1
2025-05-14 12:50:34.684866+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份连接重建成功
2025-05-14 12:50:34.684968+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份连接重建成功，继续备份过程
2025-05-14 12:50:34.685074+0800 MFCTOOL.bin[94396:4779897] [INFO] 发送备份请求 - 开始
2025-05-14 12:50:34.685178+0800 MFCTOOL.bin[94396:4779897] [INFO] 创建备份选项字典
2025-05-14 12:50:34.685286+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置目标标识符: 00008110-001025522E41801E
2025-05-14 12:50:34.685387+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份类型: 强制完全备份
2025-05-14 12:50:34.685450+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份路径: /Users/monterey/Library/Application Support/com.mfcbox.imfcdata/backups/00008110-001025522E41801E
2025-05-14 12:50:34.685498+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置DLFileOffsetKey: FALSE
2025-05-14 12:50:34.685543+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置加密标志: TRUE
2025-05-14 12:50:34.685611+0800 MFCTOOL.bin[94396:4779897] [INFO] 添加加密密码到备份选项, 密码长度: 4
2025-05-14 12:50:34.685657+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置加密类型和密钥标识符
2025-05-14 12:50:34.685705+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份应用: TRUE
2025-05-14 12:50:34.685750+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份系统文件: TRUE
2025-05-14 12:50:34.685796+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份媒体文件和相机胶卷: TRUE
2025-05-14 12:50:34.685846+0800 MFCTOOL.bin[94396:4779897] [INFO] 设置备份协议版本: 2.1
2025-05-14 12:50:34.685915+0800 MFCTOOL.bin[94396:4779897] [INFO] 完整备份选项: <?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>TargetIdentifier</key>
...
2025-05-14 12:50:34.685982+0800 MFCTOOL.bin[94396:4779897] [INFO] 发送备份请求到设备...
2025-05-14 12:50:34.686203+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份请求已成功发送
2025-05-14 12:50:34.686260+0800 MFCTOOL.bin[94396:4779897] [INFO] 发送备份请求 - 结束，结果: 成功
2025-05-14 12:50:34.686311+0800 MFCTOOL.bin[94396:4779897] [INFO] 等待0.0秒后重试
2025-05-14 12:50:36.669267+0800 MFCTOOL.bin[94396:4779897] [INFO] 收到消息类型: DLMessageDownloadFiles (#3)
2025-05-14 12:50:36.669543+0800 MFCTOOL.bin[94396:4779897] [INFO] 处理文件下载请求 (#3)
2025-05-14 12:50:36.669655+0800 MFCTOOL.bin[94396:4779897] [INFO] 处理文件下载请求
2025-05-14 12:50:36.670311+0800 MFCTOOL.bin[94396:4779897] [INFO] 文件下载请求处理完成
2025-05-14 12:50:36.673595+0800 MFCTOOL.bin[94396:4779897] [INFO] 收到消息类型: DLMessageProcessMessage (#4)
2025-05-14 12:50:36.673795+0800 MFCTOOL.bin[94396:4779897] [INFO] 处理处理消息请求 (#4)
2025-05-14 12:50:36.673890+0800 MFCTOOL.bin[94396:4779897] [WAR] 过程消息名称为空，发送通用状态响应
2025-05-14 12:50:36.688392+0800 MFCTOOL.bin[94396:4779897] [INFO] 收到消息类型: DLMessageDisconnect (#5)
2025-05-14 12:50:36.688577+0800 MFCTOOL.bin[94396:4779897] [INFO] 处理断开连接请求 (#5)
2025-05-14 12:50:36.688637+0800 MFCTOOL.bin[94396:4779897] [INFO] 接收到断开连接消息，分析备份状态
2025-05-14 12:50:36.688689+0800 MFCTOOL.bin[94396:4779897] [WAR] 收到断开连接消息，但未接收到任何文件数据
2025-05-14 12:50:36.688744+0800 MFCTOOL.bin[94396:4779897] [INFO] 检查增量备份完成状态
2025-05-14 12:50:36.688898+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份文件检查 - Manifest.plist: 存在, Info.plist: 存在, Status.plist: 存在, Manifest.db: 存在
2025-05-14 12:50:36.688950+0800 MFCTOOL.bin[94396:4779897] [INFO] 验证plist文件内容
2025-05-14 12:50:36.690437+0800 MFCTOOL.bin[94396:4779897] [INFO] 所有plist文件验证通过
2025-05-14 12:50:36.691294+0800 MFCTOOL.bin[94396:4779897] [INFO] Status.plist表明备份已完成
2025-05-14 12:50:36.692339+0800 MFCTOOL.bin[94396:4779897] [WAR] 数据库中没有文件记录
2025-05-14 12:50:36.692440+0800 MFCTOOL.bin[94396:4779897] [WAR] 可能是无效备份或空备份
2025-05-14 12:50:36.692794+0800 MFCTOOL.bin[94396:4779897] [INFO] 已发送断开响应，状态: 完成
2025-05-14 12:50:36.692908+0800 MFCTOOL.bin[94396:4779897] [INFO] 消息处理循环结束，总处理消息数: 5，总耗时: 34.4 秒
2025-05-14 12:50:36.693117+0800 MFCTOOL.bin[94396:4779897] [INFO] 消息类型统计: {
    DLMessageDisconnect = 1;
    DLMessageDownloadFiles = 2;
    DLMessageProcessMessage = 1;
}
2025-05-14 12:50:36.693198+0800 MFCTOOL.bin[94396:4779897] [INFO] 检查增量备份完成状态
2025-05-14 12:50:36.693319+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份文件检查 - Manifest.plist: 存在, Info.plist: 存在, Status.plist: 存在, Manifest.db: 存在
2025-05-14 12:50:36.693398+0800 MFCTOOL.bin[94396:4779897] [INFO] 验证plist文件内容
2025-05-14 12:50:36.693666+0800 MFCTOOL.bin[94396:4779897] [INFO] 所有plist文件验证通过
2025-05-14 12:50:36.694197+0800 MFCTOOL.bin[94396:4779897] [INFO] Status.plist表明备份已完成
2025-05-14 12:50:36.694744+0800 MFCTOOL.bin[94396:4779897] [WAR] 数据库中没有文件记录
2025-05-14 12:50:36.694816+0800 MFCTOOL.bin[94396:4779897] [WAR] 备份完成但可能无效，检查备份目录
2025-05-14 12:50:36.694897+0800 MFCTOOL.bin[94396:4779897] [ERR] 处理备份消息失败
2025-05-14 12:50:36.694950+0800 MFCTOOL.bin[94396:4779897] [ERR] 步骤4: 执行备份失败: 备份过程完成但结果可能无效，请检查
2025-05-14 12:50:36.695014+0800 MFCTOOL.bin[94396:4779897] [ERR] 备份失败: 备份过程完成但结果可能无效，请检查
2025-05-14 12:50:36.695044+0800 MFCTOOL.bin[94396:4779903] [INFO] 状态转换: 备份中 -> 错误
2025-05-14 12:50:36.695085+0800 MFCTOOL.bin[94396:4779897] [INFO] 执行资源清理
2025-05-14 12:50:36.697067+0800 MFCTOOL.bin[94396:4779897] [INFO] 资源清理完成
2025-05-14 12:50:36.697192+0800 MFCTOOL.bin[94396:4779897] [INFO] 备份任务清理完成
2025-05-14 12:50:36.697268+0800 MFCTOOL.bin[94396:4779897] [INFO] === 内部备份流程结束 ===
2025-05-14 12:50:36.697342+0800 MFCTOOL.bin[94396:4779897] [INFO] 异步备份任务完成
