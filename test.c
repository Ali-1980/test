...
// 添加到全局状态结构或客户端结构中 如果使用全局变量：
static struct {
    decrypt_key_callback_t key_callback;
    void* key_user_data;
} decrypt_key_state = { NULL, NULL };
...
int idevicerestore_set_decryption_key_callback(decrypt_key_callback_t callback, void* user_data) {
    if (!callback) {
        error("ERROR: Invalid parameters for setting decryption key callback\n");
        return -1;
    }
    decrypt_key_state.key_callback = callback;
    decrypt_key_state.key_user_data = user_data;
    debug("Decryption key callback set: %p, user_data: %p\n", callback, user_data);
    return 0;
}
...
const char* get_decryption_key(void) {
    debug("get_decryption_key: calling callback\n");
    if (!decrypt_key_state.key_callback) {
        debug("get_decryption_key: no callback set\n");
        return NULL;
    }
    return decrypt_key_state.key_callback(decrypt_key_state.key_user_data);
}
...
/**
* 设置一个回调函数，用于提供加密 IPSW 文件的解密密钥
* @param callback 返回解密密钥的回调函数
